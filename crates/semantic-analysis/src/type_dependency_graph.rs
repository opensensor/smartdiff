...
use crate::{
    TypeInfo, TypeSignature, ExtractedTypeInfo, TypeExtractionResult,
...
impl TypeDependencyGraphBuilder {
    ...
    pub fn build_from_extraction_result(&mut self, result: &TypeExtractionResult) -> Result<(), String> {
    ...
    fn add_type_relationships(&mut self, extracted_type: &ExtractedTypeInfo) -> Result<(), String> {
        ...
        for method in &extracted_type.type_info.methods {
            ...
            for parameter in &method.parameters {
                if let Some(param_type) = self.extract_parameter_type(parameter) {
    ...
    fn build_inheritance_chain(&self, start_type: &str, visited: &mut HashSet<String>) -> Vec<String> {
    ...
    fn calculate_abstractness(&self, type_name: &str) -> f64 {
        if let Some(extracted_type) = self.type_info_map.get(type_name) {
            ...
            let abstract_methods = extracted_type.type_info.methods.iter()
