...
use crate::symbol_table::{ScopeId, ScopeType, Scope, Symbol, SymbolKind};
...
impl ScopeManager {
    ...
    /// Create a new scope
    pub fn create_scope(&mut self, scope_type: ScopeType, file_path: String, start_line: usize, end_line: usize) -> ScopeId {
    ...
    pub fn resolve_symbol_from_scope(&self, name: &str, scope_id: ScopeId) -> Option<ScopeResolution> {
    ...
    pub fn is_symbol_visible(&self, symbol_name: &str, symbol_scope_id: ScopeId) -> bool {
        ...
        if let Some(current_scope_id) = self.current_scope() {
            self.is_scope_ancestor(symbol_scope_id, current_scope_id) || symbol_scope_id == current_scope_id
        } else {
            false
        }
    }
    ...
    pub fn analyze_scopes(&self) -> ScopeAnalysis {
        ...
        for scope in self.scopes.values() {
            ...
            // Count scope types
            *analysis.scope_type_counts.entry(scope.scope_type.clone()).or_insert(0) += 1;
