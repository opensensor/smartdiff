...
impl ASTProcessor {
    ...
    fn analyze_node(&self, node: &ASTNode, depth: usize, analysis: &mut ASTAnalysis, depths: &mut Vec<usize>) {
        ...
        // Count node types
        *analysis.node_type_counts.entry(node.node_type.clone()).or_insert(0) += 1;
    ...
    fn is_redundant_node(&self, node: &ASTNode) -> bool {
        match node.node_type {
            NodeType::Unknown => {
                ...
                node.metadata.attributes.get("name").is_none() &&
        ...
        while i < node.children.len() - 1 {
            ...
            if can_merge {
                ...
                if let (Some(current_text), Some(next_text)) = (
                    ...
                    current_node.metadata.attributes.insert("text".to_string(), merged_text);
                }
    ...
    fn are_mergeable_literals(&self, node1: &ASTNode, node2: &ASTNode) -> bool {
        ...
        node1.metadata.line == node2.metadata.line ||
        node1.metadata.line + 1 == node2.metadata.line
    }
    ...
    fn extract_signatures_from_node(&self, node: &ASTNode, signatures: &mut Vec<FunctionSignatureInfo>) {
        match node.node_type {
            NodeType::Function | NodeType::Method | NodeType::Constructor => {
    ...
    fn create_function_signature(&self, node: &ASTNode) -> Option<FunctionSignatureInfo> {
        ...
        let parameter_count = node.metadata.attributes.get("parameter_count")
            .and_then(|s| s.parse().ok())
            .unwrap_or(0);
        let modifiers = node.metadata.attributes.get("modifiers")
    ...
    fn collect_symbols(&self, node: &ASTNode, symbol_table: &mut SymbolTable, scope_path: Vec<String>) {
        match node.node_type {
            NodeType::Function | NodeType::Method | NodeType::Constructor => {
