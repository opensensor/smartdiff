...
use smart_diff_semantic::{FunctionSignatureExtractor, FunctionSignatureConfig};
use smart_diff_engine::{
    SimilarityScorer, SimilarityScoringConfig, MatchType,
...
"#;
    ...
    let original_signatures = signature_extractor.extract_signatures("Calculator.java", &original_parse)?;
    let modified_signatures = signature_extractor.extract_signatures("Calculator.java", &modified_parse)?;
    ...
    for original_sig in &original_signatures.signatures {
        ...
        for modified_sig in &modified_signatures.signatures {
            ...
            if let (Some(orig_ast), Some(mod_ast)) = (
                ...
                let similarity = similarity_scorer.calculate_comprehensive_similarity(
                    original_sig, orig_ast,
                    modified_sig, mod_ast
                )?;
        ...
        if let Some((similarity, matched_name)) = best_match {
            println!("    Best match: {} (similarity: {:.3})", matched_name, similarity.overall_similarity);
            println!("    Match type: {:?}", similarity.match_type);
            println!("    Confidence: {:.3}", similarity.confidence);
            
            println!("    Similarity breakdown:");
            println!("      Signature: {:.3}", similarity.signature_similarity.overall_similarity);
            println!("      AST Body: {:.3}", similarity.body_similarity.overall_similarity);
            println!("      Context: {:.3}", similarity.context_similarity.overall_similarity);
            
            if !similarity.similarity_breakdown.contributing_factors.is_empty() {
            ...
            if !similarity.similarity_breakdown.dissimilarity_factors.is_empty() {
        ...
        for (j, (name2, code2)) in functions.iter().enumerate() {
            ...
            if let (Some(sig1), Some(sig2)) = (sigs1.signatures.first(), sigs2.signatures.first()) {
                if let (Some(ast1), Some(ast2)) = (
                    ...
                    println!("      Structural similarity: {:.3}", similarity.body_similarity.structural_similarity);
                    println!("      Content similarity: {:.3}", similarity.body_similarity.content_similarity);
                    println!("      Control flow similarity: {:.3}", similarity.body_similarity.control_flow_similarity);
                    println!("      Edit distance score: {:.3}", similarity.body_similarity.edit_distance_score);
                    
                    println!("    Semantic Analysis:");
                    println!("      Algorithm pattern similarity: {:.3}", similarity.semantic_metrics.algorithm_pattern_similarity);
                    println!("      API pattern similarity: {:.3}", similarity.semantic_metrics.api_pattern_similarity);
                    
                    if !similarity.similarity_breakdown.control_flow_patterns.is_empty() {
                        println!("    Common control flow patterns: {:?}", 
...
"#;
    ...
    if let (Some(sig1), Some(sig2)) = (sigs1.signatures.first(), sigs2.signatures.first()) {
        if let (Some(ast1), Some(ast2)) = (
            ...
            println!("\n  Semantic Analysis:");
            println!("    API pattern similarity: {:.3}", similarity.semantic_metrics.api_pattern_similarity);
            println!("    Error handling similarity: {:.3}", similarity.semantic_metrics.error_handling_similarity);
            println!("    Type usage similarity: {:.3}", similarity.semantic_metrics.type_usage_similarity);
            
            println!("\n  Context Analysis:");
            println!("    Function call similarity: {:.3}", similarity.context_similarity.function_call_similarity);
            println!("    Variable usage similarity: {:.3}", similarity.context_similarity.variable_usage_similarity);
            
            if !similarity.similarity_breakdown.common_variables.is_empty() {
                println!("    Common variables: {:?}", similarity.similarity_breakdown.common_variables);
            }
        }
    }
...
fn find_function_ast<'a>(ast: &'a smart_diff_parser::ASTNode, function_name: &str) -> Option<&'a smart_diff_parser::ASTNode> {
