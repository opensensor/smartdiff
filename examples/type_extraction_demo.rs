...
use smart_diff_semantic::{
    TypeDependencyGraphBuilder, TypeEquivalence, TypeExtractor, TypeExtractorConfig,
    TypeRelationshipType, TypeSignature,
};
...
"#;
    ...
    for extracted_type in &extraction_result.types {
        ...
        println!(
            "    File: {} (line {})",
            type_info.file_path, type_info.line
        );
        ...
        if !extracted_type.generic_constraints.is_empty() {
            println!(
                "    Generic constraints: {:?}",
                extracted_type.generic_constraints
            );
        }
    }
...
"#;
    ...
    let files = vec![
        (
            "Repository.java".to_string(),
            parser.parse(interface_code, Language::Java)?,
        ),
        (
            "UserRepository.java".to_string(),
            parser.parse(implementation_code, Language::Java)?,
        ),
    ];
    ...
    if !analysis.inheritance_chains.is_empty() {
        println!(
            "  Inheritance chains: {}",
            analysis.inheritance_chains.len()
        );
