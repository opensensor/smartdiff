...
use smart_diff_semantic::{
    SymbolResolver, SymbolResolverConfig, ScopeManager, 
    SymbolKind, ScopeType, ReferenceType
};
...
"#;
    ...
    for method in &methods {
        println!("    - {} at line {} ({})", method.name, method.line, 
    ...
    for field in &fields {
        println!("    - {} at line {} ({})", field.name, field.line,
    ...
    if let Some(class_symbol) = resolver.find_symbol("DataProcessor", Some("DataProcessor.java")) {
        println!("  Found class: {} (kind: {:?})", class_symbol.name, class_symbol.symbol_kind);
    }
    
    if let Some(method_symbol) = resolver.find_symbol("processData", Some("DataProcessor.java")) {
        println!("  Found method: {} (kind: {:?})", method_symbol.name, method_symbol.symbol_kind);
    }
...
"#;
    ...
    if let Some(basic_calc_symbol) = resolver.find_symbol("BasicCalculator", None) {
        println!("Found BasicCalculator class: {} ({})", 
...
fn demo_import_resolution() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (lang, import_stmt) in import_examples {
        ...
        match result {
            Ok(import_info) => {
                println!("  {} - {}: {} (wildcard: {}, alias: {:?})", 
                         lang, import_stmt, import_info.imported_name, 
                         import_info.is_wildcard, import_info.alias);
            }
            Err(e) => {
...
fn demo_scope_management() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let global_scope = scope_manager.create_scope(
        ScopeType::Global, 
        "ScopeDemo.java".to_string(), 
        1, 
        100
    );
    scope_manager.enter_scope(global_scope);
    println!("Created global scope (depth: {})", scope_manager.current_depth());
    ...
    println!("Created class scope (depth: {})", scope_manager.current_depth());
    ...
    println!("Created method scope (depth: {})", scope_manager.current_depth());
    
    let block_scope = scope_manager.create_scope(
        ScopeType::Block, 
        "ScopeDemo.java".to_string(), 
        15, 
        25
    ...
    println!("Created block scope (depth: {})", scope_manager.current_depth());
...
"#;
    ...
    println!("  Avg references per symbol: {:.2}", stats.avg_references_per_symbol);
