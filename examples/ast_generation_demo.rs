...
use smart_diff_parser::{
    ast::NodeType, ast_processor::ASTProcessor, language::Language, parser::Parser,
...
"#;
    ...
    for signature in &signatures {
        ...
        println!(
            ...
            signature.line, signature.column
        );
        println!();
    }
...
"#;
    ...
    for (symbol_type, symbols) in &by_type {
        ...
        for symbol in symbols {
            ...
            println!(
                "  - {} (scope: {}) at line {}",
                symbol.name, scope_str, symbol.line
            );
        }
    }
...
"#;
    ...
    if nodes_saved > 0 {
        ...
        println!(
            "  Total reduction: {} nodes ({:.1}%)",
            nodes_saved, reduction_percent
        );
    }
...
"#;
    ...
    let analysis_without_comments =
        ASTProcessor::new(Language::C).analyze(&result_without_comments.ast);

    println!("  Total nodes: {}", analysis_without_comments.total_nodes);
    println!(
        "  Comment nodes: {}",
        analysis_without_comments
            .node_type_counts
            .get(&NodeType::Comment)
            .unwrap_or(&0)
    );
    ...
    let node_difference =
        analysis_with_comments.total_nodes - analysis_without_comments.total_nodes;
